...
{% for number in numbers %}
{% if (number < 0) %}
<li><p style="color:red">{{number}}</li>
{% else %}
<li><p style="color:green">{{number}}</li>
{% endif %}
{% endfor %}
...
<svg width="500" height="200">
{% for number in numbers %}
<rect x="{{loop.index * 25}}" y="{{ 50 - number }}" width="20" height="{{number}}" style="fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)" />
{% endfor %}
</svg>
...
<svg width="500" height="200">
{% for number in numbers %}
<rect y="{{loop.index * 25}}" width="{{ number }}" height="20" style="fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)" />
{% endfor %}
</svg>
...
{% for point in data %}
<circle cx="{{12 * point[0]}}" cy="{{3 * point[1]}}" r="2" stroke="black" stroke-width="3" fill="red" />
{% endfor %}
</svg>
...
<svg width="500" height="200" style="border:1px solid grey">
{% for point in data %}
<circle cx="{{12 * point[0]}}" cy="{{3 * point[1]}}" r="{{ 2 + point[2] }}" stroke="black" stroke-width="3" fill="red" />
{% endfor %}
</svg>
